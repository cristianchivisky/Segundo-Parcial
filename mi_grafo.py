...
class nodoVertice():
    info, datos, sig, visitado, adyacentes = None, None, None, False, None
...
class Arista():
    ...
    def barrido_aristas_mayor_2(self):
        lista=[]
        aux = self.__inicio
        while(aux is not None):
            if aux.peso >=2:
...
class Grafo():
    ...
    def barrido_lista_lista_2(self):
        aux = self.__inicio
        while(aux is not None):
            #print('Vertice:', aux.info)
            #print('arsitas con peso mayoro igual a 2 :')
            for arist in aux.adyacentes.barrido_aristas_mayor_2():
                print(f'{aux.info} - {arist}')
            aux = aux.sig

    def barrido_lista_lista_cant(self):
        mayor=0
        nombre_mayor=None
        aux = self.__inicio
        while(aux is not None):
            auxiliar=aux.adyacentes.barrido_aristas_cant()
            if auxiliar > mayor:
                mayor=auxiliar
                nombre_mayor=aux.info
            aux = aux.sig
        return nombre_mayor
    ...
    def contar_maravillas(self):
            ...
            aux = self.__inicio
            while aux is not None:
                paises_list = aux.datos['pais'].split('-')
                for pais in paises_list:
                    if pais not in paises:
                        paises[pais] = {'arq': False, 'nat': False}
                    
                    if aux.datos['tipo'] == 'Natural':
                        paises[pais]['nat'] = True
                    else:
